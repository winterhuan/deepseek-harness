...
**Projection intents.** `creative_production` is a concurrency-safe, side-effect-free tool whose tool call lands in the session log; the browser workbench replays it to drive the short-drama production views. It never edits creator documents or authorizes paid generation.
