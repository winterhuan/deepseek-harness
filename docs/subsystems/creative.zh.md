...
**投影意图。** `creative_production` 是并发安全、无副作用的工具，其工具调用落入会话日志；浏览器工作台重放它来驱动短剧生产视图。它从不修改创作者文档，也不为付费生成授权。待处理输入归会话的 `inbox` 投影，持久化的生产结果归 Conversation 投影，展示草稿归工作台 store。
