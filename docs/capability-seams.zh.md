...
| `ctx.skillViewerCatalog` | `core` | [`skill-viewer`](../packages/skill/skill-viewer) | - | [`client-ui-skill-viewer`](../packages/client/ui-skill-viewer) | - | 在不激活 Agent 也不写入会话事件的前提下，为人工浏览提供允许用户调用的 skill 元数据与正文。 |
