...
describe('DSH workspace text boundary', () => {
  it('allows editable text inside book directories and analysis output', () => {
    for (const path of [
      '正文/第001章.md',
      '洪荒：开天余烬/大纲/大纲.md',
      '洪荒：开天余烬/追踪/上下文.md',
    ...
    expect(() => { assertCreativePath('洪荒：开天余烬/设定/参考图.png', 'media') }).not.toThrow()
  })

  it('rejects traversal, external roots and non-editable types', () => {
    for (const path of [
      '洪荒：开天余烬/大纲/../secret.md',
      '/洪荒：开天余烬/大纲/大纲.md',
      'src/app.ts',
      'foo/bar/baz.md',
      '洪荒：开天余烬/大纲/notes.log',
    ]) {
