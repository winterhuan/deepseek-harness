...
Production cards persist request drafts per Session, not execution state. Each card has a `ProductionRequestId`; its preparation message has a separate `SessionRequestId` matched to queue occurrences by `rpcId`. The task board reconciles queued preparations against the Session's `inbox` projection, which is the only queue source: because production preparations use `queue` delivery, only pending `next-turn` occurrences become queued rows, and `next-step` steering never appears or offers withdrawal. An absent projection has not arrived yet and contributes no row. The Session-scoped injected operation owns the complete `SubmissionHandle` from `beginSubmission()`, so a preparation whose dispatch cannot reach `session.prompt()` retires its local echo through `handle.abandon()`; the view reads only the handle's `requestId`. Confirmed background executions return real `{ jobId, startedAt }` references. Incremental Conversation projection restores bindings from Native result metadata or the compact JSON content also retained by PTC, and publishes every result with its Session sequence; the workbench replay of navigation and sequence intents is a monotonic last-applied sequence cursor, so a tab remount applies only newer results instead of replaying consumed history. Only the Session Controller's `jobsBySession` supplies running, stopping and terminal states, after its complete control baseline arrives. A card can contain multiple jobs; ended registered jobs do not prove that every planned asset succeeded. The media library remains an independent file view.

Live file activity is derived from the formal Chat node store — every visible running tool root, including nested PTC children — not from the compatibility running-call slice, so one assembled tool lifecycle drives both the transcript and the workbench. Rows the Chat store marks hidden contribute nothing, which is narrower than the replaced slice: a hidden running call no longer drives workbench mutations even though the transcript still renders it.
