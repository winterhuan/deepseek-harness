...
Live file activity is derived from the formal Chat node store — every visible running tool root, including nested PTC children — so one assembled tool lifecycle drives both the transcript and the workbench. Rows the Chat store marks hidden contribute nothing, while the transcript still renders them.
...
- **Large knowledge assets are bundled** — the `knowledge/` tree increases clone size; a future iteration may fetch it on demand.
