...
生产卡片按 Session 持久化请求草稿，不保存执行状态。每张卡片拥有 `ProductionRequestId`；准备消息拥有独立的 `SessionRequestId`，通过 `rpcId` 关联队列中的消息实例。任务板按 Session 的 `inbox` 投影对账已排队的准备消息，该投影是唯一的队列来源：由于生产准备消息使用 `queue` 投递，只有待处理的 `next-turn` 实例会成为排队行，`next-step` steering 永不出现、也永不提供撤回。投影缺失表示数据尚未到达，不产生任何行、也不推断受理失败。Session 级注入操作持有 `beginSubmission()` 返回的完整 `SubmissionHandle`，因此派发无法到达 `session.prompt()` 的准备消息通过 `handle.abandon()` 撤销本地回显；视图只读取句柄的 `requestId`。确认后的后台执行返回真实的 `{ jobId, startedAt }` 引用。增量 Conversation 投影从 Native 结果元数据或 PTC 同样保留的小型 JSON 内容恢复绑定，并随每个结果发布其 Session 序列；工作台重放导航与序列意图的机制是单调的最后应用序列游标，因此标签重新挂载只应用更新的结果，而不重放已消费的历史。完整控制基线到达后，只有 Session Controller 的 `jobsBySession` 提供运行、停止中和终态。一张卡片可包含多个作业；已登记作业结束不证明全部计划素材成功。素材库仍是独立的文件视图。

活跃文件改动派生自正式的 Chat 节点 store —— 每个可见的、仍在运行的工具根调用及其嵌套 PTC 子调用 —— 而非兼容层 running-call 切片，因此同一份已装配的工具生命周期同时驱动对话记录与工作台。被 Chat store 标记为 hidden 的行不参与派生，这比被替换的切片更窄：隐藏的运行调用不再驱动工作台 mutation，尽管对话记录仍然渲染它。
