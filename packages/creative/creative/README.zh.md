...
活跃文件改动派生自正式的 Chat 节点 store —— 每个可见的、仍在运行的工具根调用及其嵌套 PTC 子调用 —— 因此同一份已装配的工具生命周期同时驱动对话记录与工作台。被 Chat store 标记为 hidden 的行不参与派生，但对话记录仍会渲染它。
...
- **大型知识库资源被打包** — `knowledge/` 树增大了克隆体积；未来迭代可能改为按需获取而不改变 provider 契约。
