...
This file is a DSH-side companion to ``provider_adapters.py`` in the same
directory: it imports that module's
shared helpers (credential handling, safe HTTP, reference inlining, output
handling) and adds the Agnes image and video adapters.
...
Accepted video tasks poll until an explicit terminal status or the deadline.
Inconclusive responses and retryable query failures never resubmit generation.
Completed tasks expose their download URL in ``metadata.url`` or top-level
``url``; ``internal_status`` does not determine completion.
"""
...
def prepare_agnes_video_request(job: Mapping[str, Any]) -> dict[str, Any]:
    """Resolve the video model and validate snapshot inputs without network I/O.

    An unset or blank model selects the free flash release. Unknown model ids
    and unsupported inputs fail before a request body is returned.
    """
    model = os.environ.get("AGNES_VIDEO_MODEL", "").strip() or AGNES_VIDEO_FLASH_MODEL
    references = _reference_paths(job)
    reference_roles = (
    ...
    return compile_agnes_video_payload(
        job,
        model=model,
        reference_urls=reference_urls,
        reference_roles=reference_roles,
...
def _run_agnes_video(job: Mapping[str, Any]) -> tuple[Path, str]:
    token = _credential("AGNES_API_KEY")
    body = prepare_agnes_video_request(job)
    model = body["model"]
    base = _base_url("AGNES_BASE_URL", AGNES_BASE_URL)
    ...
    try:
        ...
        raise AdapterFailure("Agnes polling configuration is invalid", request_id=video_id) from exc
    if interval <= 0 or deadline <= time.monotonic():
        raise AdapterFailure("Agnes polling configuration is invalid", request_id=video_id)
    delay = interval
    try:
        ...
        exc.request_id = _safe_token(video_id)
        raise
    raise AdapterFailure(
