...
MAX_KEY_ATTEMPTS = 16
BUNDLED_ADAPTERS = {
    "gpt-image-2": ("provider_adapters.py", "OPENAI_API_KEY"),
    "minimax-h3": ("provider_adapters.py", "MINIMAX_API_KEY"),
    "minimax-music": ("provider_adapters.py", "MINIMAX_API_KEY"),
    "seedance": ("provider_adapters.py", "ARK_API_KEY"),
    "agnes-image": ("agnes_adapters.py", "AGNES_API_KEY"),
    "agnes-video": ("agnes_adapters.py", "AGNES_API_KEY"),
}
...
def _validate_stored_job(
    ...
    if set(document) != STORED_JOB_KEYS:
        raise ValueError("stored job fields are invalid")
...
def _load_bundled_adapter(profile: str) -> tuple[list[str], str]:
    if profile not in BUNDLED_ADAPTERS:
        raise ValueError("unsupported bundled adapter")
    filename, credential_name = BUNDLED_ADAPTERS[profile]
    script = Path(__file__).resolve().with_name(filename)
    return [sys.executable, "-B", str(script), profile], credential_name


def _read_credential_pool() -> list[str]:
    raw = sys.stdin.buffer.read(MAX_ADAPTER_RESPONSE_BYTES + 1)
    if len(raw) > MAX_ADAPTER_RESPONSE_BYTES:
        raise ValueError("credential pool exceeds 1 MiB")
    try:
        document = json.loads(raw)
    except (ValueError, UnicodeError) as exc:
        raise ValueError("credential pool must be a JSON array") from exc
    if (
        not isinstance(document, list)
        or len(document) > MAX_KEY_ATTEMPTS
        or any(not isinstance(value, str) or not value or "\x00" in value for value in document)
    ):
        raise ValueError("credential pool must contain at most sixteen non-empty keys")
    return document
...
def _parse_public_adapter_error(
    ...
    if "submission_rejected" in error:
        if not isinstance(error["submission_rejected"], bool):
            return fallback
        result["submission_rejected"] = error["submission_rejected"]
...
def _run_adapter_with_failover(
    command: list[str],
    timeout: int,
    payload: Mapping[str, Any],
    root: Path,
    *,
    ...
    """Rotate keys only after a rejected initial submission, within one run."""
    if not credential_pool:
        return _run_adapter(command, timeout, payload, root)
    ...
    while True:
        attempt += 1
        env = {**os.environ, credential_name: credential_pool[key_index]}
        try:
            return _run_adapter(command, max(0, deadline - time.monotonic()), payload, root, env=env)
        except AdapterError as exc:
            error = exc.public_error or {}
            category = error.get("category")
            if (
                error.get("submission_rejected") is not True
                or category not in {"authentication", "permission", "rate_limit"}
                or attempt == MAX_KEY_ATTEMPTS
            ):
                raise
            if key_index + 1 < len(credential_pool):
                key_index += 1
            elif category == "rate_limit" and same_key_retries < 2:
                same_key_retries += 1
            else:
                raise
            if category == "rate_limit":
                delay = min(2 ** throttles * random.uniform(0.75, 1.25), 10)
...
def run_job(
    ...
    *,
    ...
    bundled_adapter: str | None = None,
    credential_pool: list[str] | None = None,
) -> dict[str, Any]:
    """Consume one confirmation and publish one verified run through either adapter source."""
    if (adapter_config is None) == (bundled_adapter is None):
        raise ValueError("select exactly one adapter config or bundled adapter")
    if credential_pool is not None and bundled_adapter is None:
        raise ValueError("credential pools require a bundled adapter")
    root = find_project(root)
    with tempfile.TemporaryDirectory(prefix="short-drama-inputs-") as directory:
        ...
        with _project_lock(root):
            ...
            if bundled_adapter is not None:
                if job["adapter"] != bundled_adapter:
                    raise ValueError("bundled adapter must match the prepared job")
                command, credential_name = _load_bundled_adapter(bundled_adapter)
            ...
            if bundled_adapter == "agnes-video":
                from agnes_adapters import prepare_agnes_video_request

                prepare_agnes_video_request({**job, "project_root": str(snapshot_root)})
        ...
        try:
            if credential_name is not None:
                response = _run_adapter_with_failover(
                    command, timeout, payload, root,
                    credential_name=credential_name,
                    credential_pool=credential_pool or [],
                )
            else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    adapter.add_argument("--adapter-config")
    adapter.add_argument("--bundled-adapter", choices=tuple(BUNDLED_ADAPTERS))
    run.add_argument("--credential-pool-stdin", action="store_true")
    ...
    try:
        if args.command == "prepare":
            ...
            result = run_job(
                Path(args.project),
                job_id=args.job_id,
                adapter_config=Path(args.adapter_config) if args.adapter_config else None,
                bundled_adapter=args.bundled_adapter,
                credential_pool=_read_credential_pool() if args.credential_pool_stdin else None,
            )
        ...
        if isinstance(exc, AdapterError) and exc.public_error is not None:
            print(json.dumps({"job_id": args.job_id, "state": "failed", "error": exc.public_error}, sort_keys=True))
