...
class AdapterFailure(RuntimeError):
    """A safe diagnostic; submission_rejected permits key rotation before acceptance."""
    ...
    def public(self, provider: str) -> dict[str, Any]:
        ...
        if self.submission_rejected:
            result["submission_rejected"] = True
        return result
...
def _http_failure(
    provider: str, error: urllib.error.HTTPError, *, submission: bool = False
) -> AdapterFailure:
    status = error.code
    with error:
        try:
    ...
    return AdapterFailure(
        ...
        submission_rejected=submission and status in {401, 403, 429},
    )
...
def _request_json(
    ...
    *,
    ...
    try:
        ...
        raise _http_failure(provider, exc, submission=submission and exc.url == url) from exc
...
def _run_openai(job: Mapping[str, Any]) -> tuple[Path, str | None]:
    ...
    if references:
        ...
        try:
            ...
            raise _http_failure("gpt-image-2", exc, submission=exc.url == request.full_url) from exc
        except TimeoutError as exc:
            raise AdapterFailure(
