...
- `/story-review` 或 `/story-review full` → 优先 spawn 全部 4 个 Agent；如果当前已经在子代理内，`creative_role` 不可用，或 spawn 失败，自动降级为 solo。
...
3. **确认工具可用**：full 使用 `story-architect`、`character-designer`、`narrative-writer`、`consistency-checker`，lean 使用 `story-architect`、`consistency-checker`。通过当前可见的 `creative_role` 调用；工具不可用时降级 `solo`，报告 `Fallback: agent tool unavailable -> solo`。
...
Fallback: none | agent tool unavailable -> solo | spawn failed -> solo | subagent recursion guard -> solo
...
**Agent 3: narrative-writer**（role: narrative-writer）
- full 模式调用。
...
**Agent 4: consistency-checker**（role: consistency-checker）
- full/lean 均调用。
