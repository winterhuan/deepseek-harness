...
2. `_tracking-state.json` 不存在但项目已有正文：停止日更。当前写作 workflow 只接受已初始化的追踪协议；使用 `/story-import` 重新建立写作工程。
