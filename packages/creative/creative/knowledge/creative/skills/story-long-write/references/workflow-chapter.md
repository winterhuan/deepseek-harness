...
本文件的专业 Role 均通过当前会话的 `creative_role` 调用，使用对应 `role` 和自包含 `prompt`；不检查或部署平台 Agent 文件。工具不可用或调用失败时，按本文件要求由主会话完成并说明限制。
...
2. **读取上下文**（按需选择；缺失时遵循各项及SKILL.md Phase 4 的「缺失文件处理」，仅明确标为可选的非主产物跳过。可选快捷路径：`creative_role` 可用时调用 `story-explorer` 一次获取上下文。Prompt：`项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章\n追踪状态：last_committed_chapter={check 的值}，state_revision={check 的值}`）：
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **模块召回、题材卡与文风召回**：
     ...
     - **快捷路径**：`creative_role` 可用时，可一次性召回文风/模块材料。
       - 通过 `creative_role` 调用 `story-explorer`，按下文准备 prompt。
...
4. **资料研究**（按需）：遇到需查证的外部事实且 `creative_role` 可用时，调用 `story-researcher` 搜索并输出到 `参考资料/`；不可用则主线程直接执行。研究完成后再继续写作。
...
7. **正文执行**：
   - 确认当前会话可以调用 `creative_role`。
   - 如可用，spawn `creative_role(role: "narrative-writer", prompt: ...)`，prompt 只传本章必需材料：
   ...
   - agent 在同一 session 内按上述流程写两个临时 segment；主会话确认二者只消费批准情节点后，组装并写入 `正文/第XXX章_章名.md` 一次。如 `creative_role` 不可用，由主线程执行同一流程。
8. **非对称收口**：运行 `{PYTHON} {skill 根}/scripts/storyctl.py chapter check --project {项目根} --chapter {N}`；文件变更重跑，不存决议。
   - 带内 + pass → `chapter commit`；fail / `invalid` → 停止。`TOOL_UNAVAILABLE` 或 `TOOL_ERROR` 表示检查器不可用、启动/运行失败或输出无效，不是正文已通过；修复执行问题后重跑，不据此自动重写正文。
