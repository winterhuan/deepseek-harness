...
1. **已有小说直接导入**：运行 `/story-import` 重建写作工程，再用 `/story-long-write 日更/写第N章` 或 `/story-short-write` 续写；不需要重新部署或刷新会话。
2. **需要初始化目录时**：`/story-setup` 只负责小说数据结构，不是导入或调用 Role 的前置条件。
...
Phase 2 通过 `creative_role` 调用打包的 `chapter-extractor`。工具不可用、调用失败或当前已在子代理内时，按对应 analyze skill 的同一输出和质量要求串行处理，并说明实际执行方式；不检查部署标记，也不要求用户先部署 Agent。
...
story-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并用当前可见的提问工具 询问是否继续全量拆解（对应 story-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-6 的全套产物（逐章摘要 / 聚合分析 / `剧情/节奏.md` / `剧情/情绪模块.md` / 设定关系 / 汇总报告 / 文风），缺一不可——否则 Phase 3 迁移会拿到半成品。
...
- Phase 1 确认可用工具后，通过 `creative_role` 委托 chapter-extractor；工具不可用或调用失败时，Stage 2 按同一质量要求串行处理并报告实际执行方式。
...
| Step 4：续跑检查（`拆文库/{导入书名}/_meta.json` 已存在时） | 来源与本次导入源一致且全部阶段完成时直接进 Phase 3；否则从 Stage 2 重新生成当前导入产物 |

`_meta.json` 的 `genre_detected` 由 Step 3 产出，是拆文契约的阻断级必填字段，下游 story-short-write 靠它选题材标尺——**不要跳过 Step 3 直接从原文备份起跑**。

- 措辞示例：启动深度分析时声明「《{导入书名}》篇幅已确认为短篇（题材 {题材类型}，全文约 {N} 字），Step 2 直接按短篇继续，题材识别照跑，确保 Stage 2-6 全部产出」。
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色+**关键信息与扩写技法**）。每章10-40情节点（密度150-200字/个，按字数动态调节）。角色过滤（龙套不提取、别名归类）。**并行 chapter-extractor agent 模式**（工具不可用时按同一要求串行处理）。**计数验证：摘要数 == 章节数**。 | 所有章节处理完成 |
...
- 可选验证：通过 `creative_role` 调用 `story-explorer` 交叉验证迁移数据完整性。Prompt：`项目目录：{dir}\n查询类型：progress\n查询参数：导入验证`
...
2. **追踪一次到位**：初始化事务的 `last_chapter` 写**最后一个已写完的章号 N**，不是首期拆解的 50。`imported_through_chapter` 由 `init` 一次写定、之后不再推进，逐章事务只接受 N+1 起的章号；第 1..N 章不伪造逐章记录，续写从 N+1 开始。若 init 时误写成 50，第 51..N 章仍可逐章 `append` 补上（一章一份事务，章号必须连续），只是要为已写好的章节逐章构造事务；不要删除追踪状态后重来。
...
| 数据初始化 | `/story-setup` 只初始化或校验小说数据，不部署运行环境 |

---

## 跨域导出与谱系

长篇交给 `short-drama-novel-analyze` 或需要单一原文的 `novel-game-analyze` 时，用本技能的 [export_novel_txt.py](scripts/export_novel_txt.py) 将 `正文/` 章节导出为 `原著.txt` 与 `章节映射.json`；通过 `--story-root` 指定小说工程、`--out-dir` 指定导出目录。保留章节映射以便分析片段引用回章节文件，不手工拼接。短篇直接使用其 `正文.md`。

每次跨域改编或媒体交付都用 [record_lineage.py](scripts/record_lineage.py) 记录来源与目标，传入 `--workspace-root`、`--from-domain`、`--from-path`、`--to-domain`、`--to-path`，必要时附 `--decision`。脚本在工作区根目录的 `改编谱系.jsonl` 追加包含来源指纹的记录；不得覆盖已有谱系。改编接收输入时即可记录目标项目地址，即使目标尚未创建；媒体交付边只记录实际完成的文件复制，不把计划当作交付。
...
| 项目数据结构需要初始化或校验 | story-setup | `/story-setup` |
