...
榜单与 URL 以用户提供的资料和当前可核验页面为准。

使用当前 DSH 会话可见的网页或浏览器工具，记录实际采集方式；不运行打包抓取脚本或另起 CDP。访问失败时保留缺失项，不能绕过站点限制。
...
只提取可见工具实际读到的书名、作者、简介和题材。字体混淆、缺失字段或访问限制导致内容不可核验时，标明缺口，保留作品页链接；不把推测或搜索摘要填成详情页事实。

**故障排查（书名全是 `bookId:xxx` / `（标题待解析）`）**：
- 看文件头 `数据质量`：标 `[标题解析异常]` 说明详情页解码失败率高。
- 详情页变动或登录/验证限制可能导致采集失败；使用当前可见浏览器能力核验 `https://fanqienovel.com/page/{bookId}`，不提取凭据或绕过验证。
- 当前会话没有兼容能力时，报告失败项并请用户提供可用样本，不启动 Chrome/CDP。
