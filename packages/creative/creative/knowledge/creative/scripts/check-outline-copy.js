 ...
 *
 * 退出码：0 = 干净或未找到同章细纲；1 = 有重合待复核；2 = 参数、读取或运行错误。
 * 无发现时完全静默，不污染上下文。
 */
...
function main() {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (argument === '--outline') {
      explicitOutline = argv[++index]
      if (!explicitOutline || explicitOutline.startsWith('--')) throw new Error('--outline requires a file path')
    } else if (argument === '--help' || argument === '-h') {
      process.stdout.write('用法: node check-outline-copy.js [--outline <细纲路径>] <正文路径...>\n')
      return 0
    } else if (argument.startsWith('--')) throw new Error(`Unknown option: ${argument}`)
    else proseFiles.push(argument)
  }
