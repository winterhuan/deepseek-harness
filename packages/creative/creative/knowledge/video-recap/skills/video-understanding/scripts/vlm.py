...
def _mimo_partial_source_key(video_path, scenes):
    return {
...
def _load_mimo_partial(partial_path, video_path=None, scenes=None):
    ...
    Returns {} when missing/unreadable or when the settings, source video, or scene plan differ,
    ...
    if video_path is not None or scenes is not None:
        try:
            if partial.get("source_key") != _mimo_partial_source_key(video_path, scenes):
                return {}
...
def _save_mimo_partial(partial_path, done, video_path=None, scenes=None):
    ...
    if video_path is not None or scenes is not None:
        payload["source_key"] = _mimo_partial_source_key(video_path, scenes)
