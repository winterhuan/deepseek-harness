...
In DSH, credentialed scripts run through `creative_produce_run`. Credentials and provider defaults come from the `creative-produce` profile and credential store; never export keys in a shell or put them in project files, visible tool arguments, or chat.

The table documents script-level settings, not additional tool fields. Pass supported CLI flags in `argv`; environment-only settings need runtime configuration. Do not assume every script variable is exposed by the profile, and do not create a project config file to supply it. Defaults are script defaults unless scoped to a stage.
...
| Narration block coverage | `NARRATION_COVERAGE_TARGET` / `NARRATION_BLOCK_SECONDS` | `0.7` / `9.0` | block-recap density controls |
...
| Mask source subs | `SOURCE_SUBTITLE_MASK_POLICY` / `SOURCE_SUBTITLE_MASK_RATIO` | `off` / `0.14` | masking requires an explicit policy (`opt_in`, `safe`, or `forced`) and burned recap subtitles. Passing measured `--subtitle-y-top/--subtitle-y-bot` opts in automatically. With `--no-burn-subtitles`, the MP4 stays unmasked |
