...
独立写稿时由当前 DSH Agent 对照素材完成语义复核。需要外部 MiMo 评审时，在用户已确认的 `creative_produce_run` 完整流程中使用 `--review-narration`；不直接通过 bash 调用需要凭据的 `review.py`，也不为了单独评审启动会继续 TTS/合成的编排器。已有 `narration_review.md` 时可直接读取。没有外部评审报告时明确记录未运行，当前 Agent 自查不冒充 `review.py` 的结果；下文报告处理规则只用于实际存在的报告。
