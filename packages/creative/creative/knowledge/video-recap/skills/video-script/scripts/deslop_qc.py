...
def _style_card_requirement(work_dir: Path | None) -> tuple[bool, str]:
    ...
    if work_dir is None:
        raise ValueError("deslop_qc requires a work directory")
    path = work_dir / "deslop_qc_requirements.json"
    if not path.exists():
        raise ValueError("deslop_qc_requirements.json is missing")
    data, err = _load_json(path)
    if err is not None or not isinstance(data, dict):
        raise ValueError("deslop_qc_requirements.json is invalid")
    required = data.get("style_card_required")
    if not isinstance(required, bool):
        raise ValueError("deslop_qc_requirements.json needs boolean style_card_required")
    ...
    if not path.exists():
        return {
            ...
            "message": (
                ...
                if required else
                "style_card.json is absent; this run does not require one"
            ),
        }
    data, err = _load_json(path)
    if err is not None or not isinstance(data, dict) or not data:
        return {
            ...
            "message": (
                ...
                if required else
                "style_card.json is present but malformed or empty"
            ),
            "detail": err,
        }
    return None
