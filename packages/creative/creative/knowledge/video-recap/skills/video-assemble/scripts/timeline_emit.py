    ...
    degraded = [
        {
            ...
            "reason": clip.get("source_degraded_reason") or "unknown",
        }
        for clip in video_clips
        if clip.get("source_degraded")
    ]
    if degraded:
        timeline["source_degradation"] = {"degraded": True, "degraded_clips": degraded}
        log(f"  ⚠️ 时间线源片段降级: {degraded[0]['reason']} ({len(degraded)} clip)")
    else:
        timeline["source_degradation"] = {"degraded": False}
