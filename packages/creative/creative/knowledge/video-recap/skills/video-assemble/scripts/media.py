"""Media probing and source-clip resolution for video-assemble."""
...
def _canvas_from_stream(stream, *, default_width=1280, default_height=720, default_fps=30.0):
    ...
    if dar_text and dar_text not in {"0:1", "N/A"}:
        ...
        if dar > 0 and not (rotation in {90, 270} and dar < 1.0):
            # Preserve height and adjust width for square-pixel landscape
...
def _probe_canvas(video_path, *, command_runner=run_cmd):
    ...
    For square-pixel landscape sources, these remain the raw storage dimensions.
            ...
            for c in entries:
                ...
                if not source_path or not os.path.exists(source_path):
                    if per_clip_source or multi_source:
                        ...
                        # output window — and keep every present source's real path,
                        ...
                        clips.append({"source_id": c.get("source_id"),
                                      ...
                                      "source_degraded_reason": f"missing_source_path:{source_path or 'unset'}"})
                    continue
                clips.append({"source_id": c.get("source_id"),
