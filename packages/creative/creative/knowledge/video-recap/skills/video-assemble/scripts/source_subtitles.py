...
def _source_subtitle_mask_policy(work_dir=None):
    ...
    if not raw_policy:
        ...
        raise ValueError("SOURCE_SUBTITLE_MASK_POLICY must be off, opt_in, safe, or forced")
    ...
    if raw_policy == "off":
        ...
        active = burn
        trigger = "explicit_policy_with_burned_subtitles"
        reason = "explicit policy permits masking with burned recap subtitles"
    elif raw_policy == "safe":
        active = burn and user_subtitles
