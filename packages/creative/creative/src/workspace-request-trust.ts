 ...
 * @param entry - a configured `trustedHosts` entry.
 * @throws Error when the entry is not a canonical bare `host[:port]` authority.
 */
export function assertTrustedWorkspaceAuthority(entry: string): void {
  ...
  throw new Error(`creative: trustedHosts entry ${JSON.stringify(entry)} is not a bare host[:port] authority`)
}
