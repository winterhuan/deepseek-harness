...
export function createCreativeProduceRunTool(options: ProduceToolOptions = {}): ToolDefinition {
  ...
  return defineTool({
    ...
    async execute(args, exec) {
        ...
        const jobId = jobs.start({
          ...
          run: () => {
            const done = (async (): Promise<JobOutcome> => {
              try {
                const execution = await shell.execute(shell.resolve({ ...request, env: initialEnv, signal: controller.signal }))
                proc = execution
                try {
      ...
      const execution = await shell.execute(shell.resolve({ ...request, env: initialEnv, signal: exec.signal }))
