...
/** The workspace root directory holding all video-recap projects. */
export const VIDEO_DIRECTORY = 'video-recaps'

/** One workspace file inside a video project, as the route's listing reports it. */
...
/** One playable video asset of a project, resolved through its manifest references. */
...
/** One authoritative text artifact of a project, labeled for the VideoStudio list. */
...
/** The route's projection of one video project: pipeline state, stage, and playable assets. */
...
/**
 * The project root of a workspace video path.
 * @param path - workspace-relative path.
 * @returns `video-recaps/<project>` when the path sits in a well-formed video project, else `undefined`.
 */
...
/**
 * Whether a directory is upstream's high-volume working area that stays out of
 * the workspace listing and project scans.
 * @param path - workspace-relative directory path.
 * @returns whether the directory should be pruned from traversal.
 */
...
/**
 * Whether one video-project path belongs to the small manifest-driven subset the
 * workbench shows.
 * @param path - workspace-relative path under a video project root.
 * @returns whether the file is an authoritative text artifact or a referenced video asset.
 */
...
/**
 * Project one video-recap workspace directory into the summary the workspace
 * payload carries: pipeline state, stage, next artifact, previews, artifacts.
 * @param root - the project root path, `video-recaps/<project>`.
 * @param files - the visible files of the project from the workspace listing.
 * @param metadata - the JSON metadata documents found among `files`.
 * @returns the project summary consumed by the client VideoStudio.
 */
export function summarizeVideoProject(
  ...
  // The two pre-start stages share no code: the client translates each by code.
  let stage = source === undefined ? 'awaiting-import' : 'ready-to-start'
