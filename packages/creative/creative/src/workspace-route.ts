 ...
 * @param env - process environment consulted for producer credentials.
...
export async function runVideoPreflight(
  ...
  return {
    ...
    credentials: {
      ...
      mimo: configured(env, 'MIMO_API_KEY', 'MIMO_VIDEO_API_KEY', 'MIMO_TTS_API_KEY', 'MIMO_ASR_API_KEY'),
      fish: configured(env, 'FISH_API_KEY'),
      ttsProvider: env.TTS_PROVIDER ?? 'mimo',
    },
  }
}
...
async function bundledGameExample(): Promise<GameProjectSummary> {
  ...
  const [example, verification] = await Promise.all([
  ...
  return {
    ...
    source: 'example',
    previewReady: true,
    previewUrl: `/creative/game-preview/example/${BUNDLED_GAME_EXAMPLE}/index.html`,
    previewVersion,
...
async function handle(context: Context, request: IncomingMessage, response: ServerResponse, options: WorkspaceRouteOptions): Promise<void> {
  try {
    ...
    if (url.pathname === '/creative/workspace' && request.method === 'GET') {
      ...
      send(response, 200, { cwd: realm.cwd, files, truncated: listing.truncated, games, videos, projects, mode: 'dsh-session' })
      return
    }
    ...
    if (url.pathname === '/creative/job/stop' && request.method === 'POST') {
      ...
      const status = jobs.kill(snapshot.id, agent)
