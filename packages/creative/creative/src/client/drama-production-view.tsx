...
export function DramaProductionView(props: Props) {
  ...
  const createJob = async (targetId: string, kind: 'image' | 'video', prompt: string) => {
    ...
    const job = createPendingJob({ id: randomUUID(), targetId, kind, prompt })
  ...
  const createBatch = async (kind: 'image' | 'video') => {
    ...
    const job = createPendingJob({ id: randomUUID(), targetId: batchTarget, kind, prompt: batchPrompt, expectedOutputs: candidates.length })
    commitJobs([...jobsRef.current, { ...job, status: 'awaiting_confirmation' }])
    props.onSectionChange('tasks')
    try {
...
function ShotBoard(props: Props & { readonly onCreateJob: (targetId: string, kind: 'image' | 'video', prompt: string) => Promise<void>; readonly onBatch: (kind: 'image' | 'video') => Promise<void> }) {
  ...
  if (props.production.shots.length === 0) return <section className="creative-shot-board"><MissingDocument document={`${props.production.episodeDirectory}/分镜.md`} documentPaths={props.production.documentPaths} what="镜头" skill="/short-drama-storyboard" onNavigate={props.onNavigate} /></section>
  return <section className="creative-shot-board"><div className="creative-production-actions"><button type="button" onClick={() => { void props.onBatch('image') }}>准备批量关键帧</button><button type="button" onClick={() => { void props.onBatch('video') }}>准备批量视频</button></div><div className="creative-shot-grid">{props.production.shots.map((shot) => {
    ...
    return <article className="creative-shot-card" role="button" tabIndex={0} aria-pressed={props.selectedId === shot.id} aria-label={`选中镜头 ${shot.id} ${shot.title}`} ref={props.selectedId === shot.id ? selectedRef : undefined} data-selected={props.selectedId === shot.id || undefined} key={shot.id} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); props.onSelect(shot.id) } }} onClick={() => { props.onSelect(shot.id) }}>
      ...
      <div className="creative-shot-status"><ReadinessBadge label="关键帧" ready={completeness.keyframe} /><ReadinessBadge label="运动" ready={completeness.motion} /><ReadinessBadge label="参考" ready={completeness.references} /><span>{versions.length} 版本</span></div><div className="creative-reference-links">{shot.source !== undefined && <ReferenceButton id={shot.source} production={props.production} onNavigate={props.onNavigate} />}{shot.references.map(id => <ReferenceButton id={id} production={props.production} onNavigate={props.onNavigate} key={id} />)}</div>
...
function AssetBoard(props: Props & { readonly onCreateJob: (targetId: string, kind: 'image' | 'video', prompt: string) => Promise<void> }) {
  ...
  if (assets.length === 0 && props.libraryVersions.length === 0) return <section className="creative-assets"><MissingDocument document={`${props.production.episodeDirectory}/图片提示词.md`} documentPaths={props.production.documentPaths} what="素材" skill="/short-drama-image-prompts" onNavigate={props.onNavigate} /></section>
  return <section className="creative-assets"><div className="creative-asset-grid">{assets.map((asset) => { const prompt = 'prompt' in asset ? asset.prompt : asset.description; const versions = props.versions.filter(version => version.targetId === asset.id); const selected = selectedVersionForTarget(asset.id, props.versions, props.selections, 'image'); return <article className="creative-asset-card" ref={props.selectedId === asset.id ? selectedRef : undefined} data-selected={props.selectedId === asset.id || undefined} key={asset.id}>{selected === undefined ? <div className="creative-asset-placeholder">{asset.kind === 'character' ? '人' : asset.kind === 'scene' ? '景' : asset.kind === 'prop' ? '物' : '设'}</div> : <MediaPreview version={selected} />}<div><small>{ASSET_KIND_LABEL[asset.kind]}</small><h3>{asset.title}</h3><button type="button" onClick={() => { props.onNavigate({ path: asset.path, offset: asset.offset, id: asset.id }) }}>{asset.id}</button></div>{prompt !== undefined && <p className="creative-asset-description">{prompt}</p>}<div className="creative-card-actions">{prompt !== undefined && <button type="button" onClick={() => { void props.onCreateJob(asset.id, 'image', prompt) }}>准备素材</button>}</div>{versions.length > 0 && <VersionStrip targetId={asset.id} versions={versions} selections={props.selections} onSelectionsChange={props.onSelectionsChange} />}</article> })}</div><div className="creative-media-library"><header><div><strong>项目媒体库</strong><span>{library.length}/{props.libraryVersions.length} 项 · 可跨集复用</span></div><div><input aria-label="搜索项目媒体" value={query} placeholder="搜索 ID 或路径" onChange={(event) => { setQuery(event.target.value) }} /><select aria-label="筛选媒体类型" value={kind} onChange={(event) => { setKind(event.target.value as typeof kind) }}><option value="all">全部</option><option value="image">图片</option><option value="video">视频</option></select></div></header>{referenceTarget === undefined && <p className="creative-projection-note">先在镜头页选中一个镜头，再回到这里把已有图片设为该镜头的额外参考。</p>}<div className="creative-media-library-grid">{library.map((version) => { const selected = referenceTarget !== undefined && (props.manualReferences[referenceTarget] ?? []).includes(version.id); return <article key={version.id}><MediaPreview version={version} /><strong>{version.targetId}</strong><span title={version.path}>{version.path}</span><footer>{version.path !== undefined && <button type="button" onClick={() => { props.onOpenMedia(version.path ?? '') }}>打开文件</button>}{referenceTarget !== undefined && version.kind === 'image' && <button type="button" aria-pressed={selected} aria-label={`${selected ? '取消' : '设为'} ${referenceTarget} 参考 ${version.targetId}`} onClick={() => { toggleReference(version.id) }}>{selected ? '已引用' : '作为参考'}</button>}</footer></article> })}</div></div></section>
}
...
function TaskBoard({ jobs, queue, sessionRunning, onCancel, onRemoveQueued }: {
  ...
  return <section className="creative-task-board">
    <div className="creative-projection-note">图片与视频先预检、后确认。内置契约支持 GPT Image 2 / Seedance；实际账号、模型与可用性由当前 DSH 运行环境决定。</div>
    {jobs.length === 0 ? <div className="creative-production-empty">还没有生产任务。可从镜头或素材页提交单个或批量任务。</div> : [...jobs].reverse().map((job) => {
      const queued = queuedItemForJob(job.id, queue)
      const displayStatus = queued === undefined ? STATUS_LABELS[job.status] : 'DSH Queue'
      return <article key={job.id} data-job-id={job.id} data-status={job.status}><header><strong title={job.targetId}>{job.targetId}</strong><span>{JOB_KIND_LABEL[job.kind]}</span><span>{displayStatus}</span></header><div className="creative-task-progress"><i style={{ width: `${String(job.progress)}%` }} /></div><details><summary>查看投产提示词</summary><p>{job.prompt}</p></details>{job.expectedOutputs > 1 && <small>{job.completedOutputs}/{job.expectedOutputs} 项成果</small>}{job.error !== undefined && <div className="creative-error">{job.error}</div>}{job.output !== undefined && <MediaPreview version={job.output} />}<footer>{queued !== undefined && (job.status === 'awaiting_confirmation' || job.status === 'pending' || job.status === 'running') && <button type="button" onClick={() => { void onRemoveQueued(job, queued.id) }}>从 DSH Queue 移除</button>}{activeJobId === job.id && <button type="button" onClick={() => { void onCancel(job) }}>停止当前 DSH Turn</button>}</footer></article>
    })}
  </section>
}
...
function SequenceBoard(props: Props & { readonly onCompose: () => void }) {
  ...
  return <section className="creative-sequence">
    <div className="creative-sequence-summary">
      <strong>{props.sequence.length} 个镜头</strong>
      <span>{props.sequence.length === 0 ? '还没有镜头' : issues.length === 0 ? '已可合成' : `${String(issues.length)} 个阻塞项`}</span>
      <button type="button" disabled={issues.length > 0 || props.sequence.length < 2} onClick={props.onCompose}>合成成片</button>
    </div>
    {issues.length > 0 && <ul className="creative-sequence-issues">
      {issues.slice(0, 3).map(issue => <li key={issue}>{issue}</li>)}
      {issues.length > 3 && <li>另有 {issues.length - 3} 个阻塞项，请在下方镜头行补齐视频。</li>}
    </ul>}
    <ol>{props.sequence.map((item, index) => {
      ...
      return <li key={item.shotId}>
        ...
        {version === undefined
          ? <div className="creative-sequence-missing">缺少视频</div>
          : <MediaPreview version={version} interactive={false} />}
        <strong>{item.shotId}</strong>
        <div>
          <button type="button" aria-label={`上移 ${item.shotId}`} disabled={index === 0} onClick={() => { move(index, -1) }}>↑</button>
          <button type="button" aria-label={`下移 ${item.shotId}`} disabled={index === props.sequence.length - 1} onClick={() => { move(index, 1) }}>↓</button>
        </div>
      </li>
    })}</ol>
  </section>
