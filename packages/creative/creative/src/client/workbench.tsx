...
function CreativeWorkspace({
  t, sessionId, useSession, useSessions, useChat, useStore, actions,
  useTabInfo, useSaves, useProduction, consumeFileNavigation, beginFileSave, endFileSave,
  ...
  const productionQueue = useSession(snapshot => snapshot.queue)
  ...
  return <CreativeWorkbench
    ...
    jobsReady={jobState.controlReady}
    workspace={workspace}
    error={error}
  ...
  context.slots.inject('sidebar.right.pane.tab', function* () {
    ...
    yield context.slots.register({
      ...
      inject: (sessionId): ProductionConversationFace => {
        ...
        return {
          hooks: { saves: saves.active, production: context.uiConversation.binding(sessionBinding()).target('creative-production') },
          ...
          beginProductionSubmission: prompt => sessionBinding().session.beginSubmission({ text: prompt, attachments: [], mode: 'queue' }).requestId,
          sendProductionPrompt: async (prompt, requestId) => {
            const result = await sessionBinding().session.prompt([{ type: 'text', text: prompt }], 'queue', undefined, requestId)
