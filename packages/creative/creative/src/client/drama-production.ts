...
/**
 * Parse 视觉设定.md into visual assets, deriving slug ids when the creator did
 * not declare a stable `VISUAL-*` id.
 * @param path - the document's workspace path, recorded on every asset.
 * @param content - the document text.
...
function validateProductionProtocol(input: {
  ...
  for (const [id, items] of byId) {
    ...
    for (const item of items) add({ severity: 'error', code: 'duplicate_id', path: item.path, offset: item.offset, targetId: id, messageKey: 'diagnostic.duplicate_id', params: { id } })
  }

  for (const visual of input.visualAssets) {
    if (visual.stableId) continue
    add(visual.declaredId === undefined
      ? { severity: 'warning', code: 'generated_visual_id', path: visual.path, offset: visual.offset, targetId: visual.id, messageKey: 'diagnostic.generated_visual_id', params: { title: visual.title } }
      : { severity: 'warning', code: 'invalid_visual_id', path: visual.path, offset: visual.offset, targetId: visual.id, messageKey: 'diagnostic.invalid_visual_id', params: { title: visual.title, id: visual.declaredId } })
  }
  ...
  for (const shot of input.shots) {
    for (const reference of shot.references) {
      if (!knownReferences.has(reference)) add({ severity: 'warning', code: 'unknown_reference', path: shot.path, offset: shot.offset, targetId: shot.id, messageKey: 'diagnostic.unknown_reference', params: { id: shot.id, reference } })
    }
    if (shot.source !== undefined && sectionTarget(screenplayPath, input.documents[screenplayPath] ?? '', shot.source) === undefined) {
      add({ severity: 'warning', code: 'unknown_source', path: shot.path, offset: shot.offset, targetId: shot.id, messageKey: 'diagnostic.unknown_source', params: { id: shot.id, source: shot.source } })
    }
  }
  ...
  for (const motion of input.motions) {
    if (motion.shotId === undefined) {
      add({ severity: 'warning', code: 'motion_without_shot', path: motion.path, offset: motion.offset, targetId: motion.id, messageKey: 'diagnostic.motion_without_shot', params: { id: motion.id } })
      continue
    }
    if (!shots.has(motion.shotId)) add({ severity: 'error', code: 'unknown_motion_shot', path: motion.path, offset: motion.offset, targetId: motion.id, messageKey: 'diagnostic.unknown_motion_shot', params: { id: motion.id, shot: motion.shotId } })
  ...
  for (const [shotId, motions] of motionsByShot) {
    ...
    for (const motion of motions) add({ severity: 'error', code: 'multiple_motions', path: motion.path, offset: motion.offset, targetId: motion.id, messageKey: 'diagnostic.multiple_motions', params: { shot: shotId } })
  }
  ...
  for (const [path, prefix] of [
    ...
    for (const section of levelTwoSections(input.documents[path] ?? '')) {
      if (section.heading.toLocaleUpperCase().startsWith(prefix) && !parsedOffsets.has(`${path}:${String(section.offset)}`)) {
        add({ severity: 'error', code: 'malformed_heading', path, offset: section.offset, messageKey: 'diagnostic.malformed_heading', params: { heading: section.heading.trim(), prefix } })
      }
    }
  }
