...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    'creative.workspace': { kind: 'single'; scope: 'session' }
  }
}
...
function useWorkspace(sessionId: string): {
  ...
  const [failure, setFailure] = useState<{ readonly status: number; readonly message: string }>()
      ...
      .catch((reason: unknown) => {
        if (!controller.signal.aborted) {
          setFailure(reason instanceof WorkspaceRequestError
            ...
            : { status: 0, message: reason instanceof Error ? reason.message : String(reason) })
        }
      })
      .finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => { controller.abort() }
  }, [sessionId, version])
  const sessionUnavailable = failure !== undefined && isSessionUnavailable(failure.status)
  return { workspace, error: failure?.message, sessionUnavailable, loading, reload }
}
...
function CreativeWorkbench({
  ...
  useEffect(() => {
    ...
    for (const { intent } of pending) {
      ...
      if (intent.action === 'open_section') setProductionSection(intent.section ?? 'shots')
        ...
        const jobId = intent.jobId as string
        const targetId = intent.targetId as string
        const jobKind = intent.jobKind as ProductionJobKind
    ...
    actions, expandPath, productionIntentCalls, productionIntents, setEditorMode,
      ...
      for (const [path, value] of Object.entries(current)) {
        ...
        if (value.source === 'human' && value.content !== value.saved) {
          if (value.missing !== true) {
            result[path] = { ...value, missing: true, error: '文件已从 workspace 移除。本地草稿仍保留，可复制后放弃草稿。' }
            changed = true
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const followOfficialFileLink = (event: MouseEvent): void => {
      ...
      if (control === null || control.closest('.creative-split-surface') !== null) return
  ...
  return <div ref={surfaceRef} className="creative-split-surface" data-open="true" data-workbench={workbench}>
    ...
    {workbench === 'game' && workspace === undefined && <main id={compactStudioId} className="oh-game-studio" role="tabpanel" aria-labelledby={`${compactTabsId}-studio-tab`}><div className="oh-game-design-empty">{sessionUnavailable ? <SessionUnavailableNotice /> : (error ?? '正在连接游戏工作台…')}</div></main>}
    ...
    {workbench !== 'game' && workbench !== 'video' && <>
      <aside className="creative-tree">
        ...
        {sessionUnavailable ? <SessionUnavailableNotice /> : error !== undefined && <div className="creative-error">{error}</div>}
        {workspace?.metadataErrors.map(message => <div className="creative-warning" key={message}>{message}</div>)}
      ...
      <main className="creative-editor">
        <header>
          ...
          <div className="creative-editor-actions">
            {(previewable || productionAvailable) && !selectedMedia && <div className="creative-editor-tabs" role="tablist" aria-label={productionAvailable ? '短剧文档查看方式' : markdown ? 'Markdown 查看方式' : 'JSONL 查看方式'}>
        ...
        {activity !== undefined && activityPath !== undefined && activityPath === selected && <div className="creative-stream" data-stage={activity.stage} role="status" aria-live="polite">● {activity.stage === 'running' ? 'Agent 正在应用修改' : 'Agent 正在生成文件内容'}</div>}
        ...
        {selected === undefined
          ...
          : selectedMedia
            ...
            : buffer === undefined
              ? <div className="creative-empty">正在加载 {selected}…</div>
              : buffer.missing === true
                ? <div className="creative-empty">文件已从 workspace 移除，本地草稿仍保留。请先复制需要的内容，再放弃草稿。<button type="button" onClick={() => {
                  setBuffers((current) => {
...
type WorkbenchSlotProps = PropsRuntime<'creative.workspace'> & PropsStore<ReturnType<typeof createWorkbenchStore>> & ProductionConversationFace

/** Mount beside the official conversation without replacing Chat or Composer. */
function CreativeSplitBridge({
  sessionId, useSession, useChat, useStore, actions,
  sendProductionPrompt, cancelProduction, removeQueuedProduction,
}: WorkbenchSlotProps) {
  ...
  useLayoutEffect(() => {
    ...
    const publishLayout = (): void => {
      ...
      scroller.style.setProperty('--creative-scroll-height', `${String(scroller.clientHeight)}px`)
      ...
      scroller.style.setProperty('--creative-composer-height', `${String(composerSeat()?.getBoundingClientRect().height ?? 0)}px`)
      scroller.dataset.creativeWorkbench = workbench
      const studioPane = workbench === 'video' ? videoPane : gamePane
      scroller.dataset.creativeStudioPane = studioPane
      ...
      if (scroller.dataset.creativeLayout !== layout) scroller.dataset.creativeLayout = layout
    ...
    return () => {
      observer.disconnect()
      seats.disconnect()
      scroller.style.removeProperty('--creative-scroll-height')
      scroller.style.removeProperty('--creative-composer-height')
      delete scroller.dataset.creativeLayout
...
type WorkbenchSeatProps = PropsRuntime<'shell.overlay'> & PropsRenderSlots<'creative.workspace'>

function WorkbenchSeat({ SessionProvider, renderSlot }: WorkbenchSeatProps) {
  return <SessionProvider>{renderSlot('creative.workspace', {})}</SessionProvider>
}
...
export function apply(context: ClientContext): void {
  context.slots.inject('shell.overlay', () => {
    const disposeSeat = context.slots.register({
      ...
      order: -100,
      children: { 'creative.workspace': { kind: 'single', scope: 'session' } },
    }, WorkbenchSeat)
