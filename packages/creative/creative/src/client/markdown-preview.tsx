...
export function MarkdownPreview({ content, label, t }: {
  readonly content: string
  readonly label: string
  readonly t: TranslateNS<typeof NS>
}) {
  const parsed = parseMarkdownBlocks(content)
  if (parsed.length === 0) return <div className="creative-markdown-empty">{t('markdown.empty')}</div>
  return <article className="creative-markdown" aria-label={`${label} ${t('markdown.preview')}`}>
    {parsed.map((block, index) => {
      ...
      if (block.kind === 'ul' || block.kind === 'ol') {
        const items = block.items.map((item, itemIndex) => <li key={`${key}-${String(itemIndex)}`} className={item.checked === undefined ? undefined : 'creative-task-item'}>
          {item.checked === undefined ? null : <input type="checkbox" checked={item.checked} readOnly disabled aria-label={item.checked ? t('markdown.task.done') : t('markdown.task.pending')} />}
