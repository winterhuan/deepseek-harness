...
declare module '@deepseek-ai/dsh-client-ui-conversation/client' {
  interface ConversationViewSnapshotMap {
    /** Successful production requests and real job bindings in durable result order. */
    'creative-production': readonly SettledProductionIntent[]
  }
}

interface ProductionNode extends ConversationViewNode {
  readonly data: { readonly seq: number; readonly intent: SettledProductionIntent }
}
...
function node(context: ConversationNodeContext, intent: SettledProductionIntent, seq: number): ProductionNode {
  return {
...
export const productionDefinition: ConversationNodeDefinition<{
  ...
  start: (_context, match) => ({ intent: decode(match.event), seq: match.event.seq }),
  update: context => context.state,
  buildViewNode: context => context.state?.intent === undefined ? null : node(context, context.state.intent, context.state.seq),
}
...
export const productionCallDefinition: ConversationNodeDefinition<NativeIntentState> = {
  ...
  start: (_context, match) => {
    ...
    return {
      ...
      callTime: match.event.time, seq: match.event.seq, intent: undefined,
    }
  },
  update: (context, match) => {
    const intent = decode(match.event, context.state.call, context.state.callTime)
    return { ...context.state, seq: match.event.seq, intent: intent?.binding === undefined ? intent : undefined }
  },
  buildViewNode: context => context.state?.intent === undefined ? null : node(context, context.state.intent, context.state.seq),
}
...
export const productionView: ConversationViewDefinition<ProductionNode, readonly SettledProductionIntent[]> = {
  ...
  create: () => {
    ...
    const project = (): readonly SettledProductionIntent[] => {
      current = [...nodes.values()].sort((left, right) => left.data.seq - right.data.seq).map(node => node.data.intent)
      return current
    }
    return {
      empty: current,
