...
export async function createCreativeRoleTool(subagents?: CreativeRoleSubagents): Promise<ToolDefinition> {
  ...
  return defineTool({
    ...
    description: 'Run one of the seven bundled novel-writing Roles as a child of the current DSH Agent. Load workflow Skills such as short-drama-develop with the skill tool; Skill names are not Role names. The child inherits DSH model, workspace, permissions, lifecycle, and UI.',
    parameters: {
      role: {
        type: 'string',
        required: true,
