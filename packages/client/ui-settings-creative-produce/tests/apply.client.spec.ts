...
import type { CreativeProduceCardFace } from '../src/client/index.ts'
...
describe('ui-settings-creative-produce apply', () => {
  ...
  it('registers the page while the Host serves the namespace, titled in the active locale', async () => {
    const { ctx, slots } = await bench(['creative-produce'])
    declareRoot(slots)
    ...
    expect(entry.options).toMatchObject({ id: 'creative-produce', order: 50 })
    expect(resolveSlotLabel(entry.options.label)).toBe('创意生产')
    expect(entry.locale).toBe(NS)
    const face = (entry.inject as () => Pick<CreativeProduceCardFace, 'hooks'>)()
  ...
  it('re-reads the credentials when the Host reports a watched reference changed, and ignores another', async () => {
    const { ctx, slots, describeCredentials, remote } = await bench(['creative-produce'])
    declareRoot(slots)
    await ctx.plugin({ inject: [...inject], apply }).await()
