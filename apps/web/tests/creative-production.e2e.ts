...
describe.skipIf(MODE === 'record' || process.platform === 'win32')('web profile: Creative process jobs', () => {
  ...
  /**
   * Reach the workbench, which a saved layout may have reopened by itself.
   *
   * The expand control exists only while the column is collapsed, so a reload
   * that restores an expanded column has no way in through it. Wait for whichever
   * of the two the restore left — neither is in the document before it settles —
   * and only then take the collapsed column's route through the guide.
   */
  async function openEpisode(book: string): Promise<void> {
    const workbench = page.locator('.creative-workspace')
    const expand = page.locator('[data-sidebar-right-expand]')
    await expect.poll(
      async () => (await workbench.count()) + (await expand.count()),
      { timeout: 30_000 },
    ).toBeGreaterThan(0)
