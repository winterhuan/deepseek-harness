...
页头列表只读。每个任务的流式输出是独立功能；[Creative 按 Session 授权的停止操作](2026-09-03-creative-workbench.zh.md#production-and-credentials) 使用同一注册表与控制订阅，不扩展列表 API。
...
**页头列表不提供中断。** `kill()` 把终态投递标为已上报，因此停止本身不通知模型结果。Creative 的独立停止操作接受这一框架行为，不注入模型可见结果。
...
**`stopping` 是注册表状态。** 模型的 `job_kill` 和 Creative 的人工停止操作都能产生它。界面等待注册表结算，不根据已受理的停止请求预测取消结果。
