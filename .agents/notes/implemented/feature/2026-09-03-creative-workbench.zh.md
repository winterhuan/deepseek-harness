...
只有 `jobsBySession` 在完整 `controlReady` 基线后提供执行状态。控制流丢失会使就绪状态失效；连接就绪通知不会丢弃已接受的基线。历史未绑定请求仍是请求，Host 重启后无法匹配的作业显示不可用，不视为完成或自动重启。一个卡片可拥有多个作业；结束作业数、对话 Turn、文件或估计百分比都不能证明计划成果已成功生成。前台结果分别保留退出码、超时、信号和有界输出；非零或未知退出码使生产失败。
